 ...
 */
public class PayAddressActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener, View.OnClickListener {
...
//                }
            ...
            public void onFailure(String url, String error) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtil.initToast(PayAddressActivity.this, "获取失败，刷新重试");
                        dismissLoading();
                    }
                });
            }
...
//                        @"isdefault" : address.isdefault                // 品牌id（可选）
                        ...
                        OkHttpUtil.postJson(Constant.URL.DeleteAddress, builder, new OkHttpUtil.OnDataListener() {
                            ...
                            public void onResponse(String url, String json) {
                                ...
                                Log.e("loge", json);
                                dismissLoading();
                                ToastUtil.initToast(PayAddressActivity.this,addressEntity.getMsg().equals("success")? "成功":"失败");
                                loadData();
                            }
            ...
            case R.id.ll_addressEdit://编辑
                ...
                intent.putExtra("name",list.get((Integer) v.getTag()).getRecipientName());
                intent.putExtra("phone",list.get((Integer) v.getTag()).getRecipientPhone());
                intent.putExtra("isdefault",list.get((Integer) v.getTag()).getIsdefault());
