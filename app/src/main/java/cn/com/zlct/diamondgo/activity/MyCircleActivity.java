...
public class MyCircleActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    List<GetRecomList.DataEntity> list;
    ...
    protected void init() {
        ...
        ActionBarUtil.initActionBar(getSupportActionBar(), "分销团队", new View.OnClickListener() {
            @Override
        ...
        recyclerView.addItemDecoration(new DividerGridItemDecoration(this, myCircleAdapter.getHeaderCount()));
    ...
    protected void loadData() {
        OkHttpUtil.postJson(Constant.URL.getRecom, "phone", userInfoEntity.getPhone(), new OkHttpUtil.OnDataListener() {
            @Override
            public void onResponse(String url, String json) {
                Log.e("loge", json);

                GetRecomList collectionList = gson.fromJson(json, GetRecomList.class);
                dismissLoading();
                list = new ArrayList<>();
                list.addAll(collectionList.getList());
                myCircleAdapter.setData(list);
            ...
            public void onFailure(String url, String error) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ToastUtil.initToast(MyCircleActivity.this, "获取失败，刷新重试");
                        dismissLoading();
                    }
                });
            }
